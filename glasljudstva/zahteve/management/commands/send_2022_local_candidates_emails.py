...
def send_email(email_id, contact_id, municipality, username):
    ...
    response, response_status = mautic_api.sendEmail(
        ...
        data={"tokens": {"municipality": municipality, "username": username}},
    )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        parties = Party.objects.filter(
            election=election, mautic_id__isnull=False
        ).exclude(email="")
        ...
        for party in parties:
            ...
            self.stdout.write(
                f'Sending to {i}/{count} "{party.party_name}" mautic id = {party.mautic_id}'
            )
            time.sleep(1)
            response_status = send_email(
                email_id=EMAIL_TEMPLATE_ID,
                contact_id=party.mautic_id,
                municipality=party.municipality.name,
