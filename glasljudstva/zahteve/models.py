...
class Party(models.Model):
    ...
    party_name = models.TextField(
        blank=True, verbose_name="Ime kandidata_ke (ali stranke)"
    )
...
class DemandAnswer(models.Model):
    ...
    comment = models.CharField(
        blank=True, default="", max_length=1024, verbose_name="Komentar"
    )
    party = models.ForeignKey(
        "Party", on_delete=models.CASCADE, verbose_name="Kandidat_ka (ali stranka)"
    )
    demand = models.ForeignKey(
        "Demand", on_delete=models.CASCADE, verbose_name="Vprašanje"
    )
...
class DemandState(models.Model):
    ...
    order = models.IntegerField(
        verbose_name="Vrstni red", validators=[MinValueValidator(1)], default=1
    )
...
class MonitoringReport(Timestampable):
    ...
    # updated_at
    demand = models.ForeignKey(
        "Demand", on_delete=models.CASCADE, verbose_name="Predvolilna zaveza"
    )
    responsible_state_bodies = models.ManyToManyField(
        StateBody, verbose_name="Državni organ(i), pristojni za uresničevanje zaveze"
    )
    present_in_coalition_treaty = models.TextField(
        blank=True,
        verbose_name="Je predvolilna zaveza vključena v koalicijsko pogodbo?",
        choices=YES_NO_PARTIALLY_OPTIONS,
    )
    cooperative = models.TextField(
        ...
        choices=YES_NO_PARTIALLY_OPTIONS[:-1],
    )
